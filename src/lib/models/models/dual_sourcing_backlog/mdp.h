...
namespace DynaPlex::Models {
    namespace dual_sourcing_backlog {
        ...
        class MDP {
            ...
            // Rollout-control knobs consulted via GetH/GetM/GetL below - configurable so
            // sample-generation cost (which dominates DCL wall time here: rollout comparisons
            // scale as num_candidates * rollout_M * rollout_H per decision) can be tuned without
            // recompiling. Defaults preserve the values that were previously hardcoded.
            int64_t rollout_H = 20;
            int64_t rollout_M = 500;
            int64_t rollout_L = 100;
            int64_t rollout_reinitiate_counter = 100;
